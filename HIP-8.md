...
The operator address given must be the address of the key the operator has signed with. So, current operator will specify HYP_AVSOPERATOR_KEY=0x1234... and the operator will sign with this key. Ideally, we would like to read the key generated by the Eigenlayer CLI directly and we'll need to map from validator key to operator key in the destination chain ISM. If not, it's preferred to maintain the validator key as the operator key as that is stored in the destination chain ISM.
